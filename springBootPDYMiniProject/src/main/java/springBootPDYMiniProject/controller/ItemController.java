...
import springBootPDYMiniProject.service.item.CartListService;
import springBootPDYMiniProject.service.item.GoodsCartDelsService;
import springBootPDYMiniProject.service.item.GoodsWishListService;
...
public class ItemController {
	...
	public String buyItem(CartCommand cartCommand, HttpSession session, HttpServletResponse response) {
		...
		if (result == "000") {
			...
			String str = "<script>" + "alert('관리자는 구매할 수 없습니다.');" + "location.href='/corner/detailView/"
					+ cartCommand.getGoodsNum() + "';" + "</script>";
			try {
		 ...
		 * "redirect:/purchase/goodsBuy?nums="+cartCommand.getGoodsNum(); else if(result
		 * == "000") return "redirect:/"; else if(result == "900") return
		 * "redirect:/corner/detailView/"+cartCommand.getGoodsNum();
		 */
	}
